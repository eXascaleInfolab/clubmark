...
# Note: python3 or python[2] can be used instead of pypy
docker run -it -u `id -u $USER` -w ${WORK_DIR} -v `pwd`:${WORK_DIR} --entrypoint pypy luaxi/pycabem:env-U16.04-v2.0 ./benchmark.py "$@"
