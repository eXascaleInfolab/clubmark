...
_ADJUST_WORKERS_BY_RAM = True  # Adjust the number of worker processing depending on the remained free RAM, which is actual when computations should avoid swap usage
...
class ExecPool(object):
	...
	def __init__(self, workers=cpu_count(), afnstep=None, inramproc=False):
		...
		self._latency = 2  # sqrt(self._workersLim)  # 1 sec of sleep on pooling
